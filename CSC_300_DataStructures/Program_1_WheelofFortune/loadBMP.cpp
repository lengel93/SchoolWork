...
bool loadBMP( std::string filename, int &Rows, int &Cols, byte* &iPtr )
{
    ...
    if ( bChar == 'B' && mChar == 'M' ) // should start with "BM" for "BitMap"
    {
        ...
        infile.get(temp, 3);
        infile.read((char*)&bitsPerPixel, 4); //Samuel Patzer fixed this line because it was invalid
    ...
    {
        infile.close();
        std::cerr << "Not a valid 24-bit bitmap file: " << filename << " Cols " << Cols << " Rows " << Rows << " bitsPerPixel "<< bitsPerPixel << std::endl;
        return false;
    }
